var cheerio = require('cheerio'), email = require('emailjs'), get = require('./configFR.json'), mysql = require('mysql'), fs = require('fs'), request = require('request'), Q = require('q'), _ = require('lodash'), htmlencode = require('htmlencode');

var diff;
var requestCookie = "asos=topcatid=1000&PreferredSite=www.asos.fr&currencyid=19";
var timeStamp = 0;
var thresh = 0;
...
function timed() {
	try {
		absentConf = fs.existsSync('numTrackerFR.json');
	} catch(e) {
		absentConf = false;
		console.log(e);
	}
	if (absentConf == true) {
		try {
			absentConf = fs.existsSync('linksFR.json');
		} catch(e) {
			absentConf = false;
			console.log(e);
		}
	...
	if (absentConf == true && get.recover == true) {
		...
		var encode = JSON.parse(fs.readFileSync("numTrackerFR.json", "utf-8"));
		callFinal = !callFinal;
		if (encode.data > 0) {
...
function dataScrape(base, k, referenceArr) { {
		...
		if (get.method == "generic" || get.method == "asos") {
			if (k < get.searches.length - 1) {
				...
				var urlPrepend = "http://www.asos.fr/pgeproduct.aspx?";
				...
				fs.writeFileSync("linksFR.json", '{"data":' + JSON.stringify(mergeCats(processArr)) + '}');
				//fs.writeFileSync("linksFR.json", '{"data":' + JSON.stringify(eliminateDuplicates(resultsArr)) + '}');
				fs.writeFileSync("numTrackerFR.json", '{"data":' + "0" + '}');
				writingToEndPoint(0);
				return;
			}
...
function writingToEndPoint(startFrom) {
	...
	var links = JSON.parse(fs.readFileSync("linksFR.json", "utf-8"));
	...
	asyncLoop(start, function(loop) {
			...
			if (!error && response.statusCode == 200) {
				...
				if (get.writeToDB == true && get.method == "asos" && (jsonResults.length >= resultSpacing || l + startFrom == links.length - 1)) {
					Q(jsonResults).then(function(results) {
						...
						fs.writeFileSync("numTrackerFR.json", '{"data":' + (l + startFrom) + '}');
						jsonResults = [];
						loop.next();
					});
		...
		cleanDB(0);
		callFinal = !callFinal;
		console.log(fs.existsSync('numTrackerFR.json') + "," + fs.existsSync('linksFR.json'));
		if (fs.existsSync('numTrackerFR.json')) {
			fs.unlinkSync('numTrackerFR.json');
		}
		if (fs.existsSync('linksFR.json')) {
			fs.unlinkSync('linksFR.json');
		}
...
//			fs.writeFileSync("numTrackerFR.json", '{"data":' + inputCount + '}');
