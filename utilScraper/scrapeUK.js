var cheerio = require('cheerio'), email = require('emailjs'), get = require('./configUK.json'), mysql = require('mysql'), fs = require('fs'), request = require('request'), Q = require('q'), _ = require('lodash'), htmlencode = require('htmlencode');

var diff;
var requestCookie = "asos=topcatid=1000&PreferredSite=www.asos.com&currencyid=1";
var timeStamp = 0;
var thresh = 0;
...
function timed() {
	try {
		absentConf = fs.existsSync('numTrackerUK.json');
	} catch(e) {
		absentConf = false;
		console.log(e);
	}
	if (absentConf == true) {
		try {
			absentConf = fs.existsSync('linksUK.json');
		} catch(e) {
			absentConf = false;
			console.log(e);
		}
	...
	if (absentConf == true && get.recover == true) {
		...
		var encode = JSON.parse(fs.readFileSync("numTrackerUK.json", "utf-8"));
		callFinal = !callFinal;
		if (encode.data > 0) {
...
function dataScrape(base, k, referenceArr) { {
		...
		if (get.method == "generic" || get.method == "asos") {
			if (k < get.searches.length - 1) {
				...
				fs.writeFileSync("linksUK.json", '{"data":' + JSON.stringify(mergeCats(processArr)) + '}');
				//fs.writeFileSync("linksUK.json", '{"data":' + JSON.stringify(eliminateDuplicates(resultsArr)) + '}');
				fs.writeFileSync("numTrackerUK.json", '{"data":' + "0" + '}');
				writingToEndPoint(0);
				return;
			}
...
function writingToEndPoint(startFrom) {
	...
	var links = JSON.parse(fs.readFileSync("linksUK.json", "utf-8"));
	...
	asyncLoop(start, function(loop) {
			...
			if (!error && response.statusCode == 200) {
				...
				if (get.writeToDB == true && get.method == "asos" && (jsonResults.length >= resultSpacing || l + startFrom == links.length - 1)) {
					Q(jsonResults).then(function(results) {
							...
							promise = promise.then(function() {
								return Q.all(_.map(product_group, function(product) {
									...
									return query('INSERT INTO products (' + 'id, timestamp, gender, ' + 'name, image, description' + ') VALUES (' + ':id, FROM_UNIXTIME(:timestamp), :gender, ' + ':name, :image, :description' + ') ON DUPLICATE KEY UPDATE id = :id, timestamp = FROM_UNIXTIME(:timestamp),gender=:gender, name = :name, description = :description, image = :image', product).then(function() {
										...
										// (assume GBP as currency for now; changed to GBP)
						...
						fs.writeFileSync("numTrackerUK.json", '{"data":' + (l + startFrom) + '}');
						jsonResults = [];
						loop.next();
					});
		...
		cleanDB(0);
		callFinal = !callFinal;
		console.log(fs.existsSync('numTrackerUK.json') + "," + fs.existsSync('linksUK.json'));
		if (fs.existsSync('numTrackerUK.json')) {
			fs.unlinkSync('numTrackerUK.json');
		}
		if (fs.existsSync('linksUK.json')) {
			fs.unlinkSync('linksUK.json');
		}
...
//			fs.writeFileSync("numTrackerUK.json", '{"data":' + inputCount + '}');
