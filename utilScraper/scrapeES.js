var cheerio = require('cheerio'), email = require('emailjs'), get = require('./configES.json'), mysql = require('mysql'), fs = require('fs'), request = require('request'), Q = require('q'), _ = require('lodash'), htmlencode = require('htmlencode');

var diff;
var requestCookie = "asos=topcatid=1000&PreferredSite=www.asos.com/es/&currencyid=19";
var timeStamp = 0;
var thresh = 0;
...
function timed() {
	try {
		absentConf = fs.existsSync('numTrackerES.json');
	} catch(e) {
		absentConf = false;
		console.log(e);
	}
	if (absentConf == true) {
		try {
			absentConf = fs.existsSync('linksES.json');
		} catch(e) {
			absentConf = false;
			console.log(e);
		}
	...
	if (absentConf == true && get.recover == true) {
		...
		var encode = JSON.parse(fs.readFileSync("numTrackerES.json", "utf-8"));
		callFinal = !callFinal;
		if (encode.data > 0) {
...
function dataScrape(base, k, referenceArr) { {
		...
		if (get.method == "generic" || get.method == "asos") {
			if (k < get.searches.length - 1) {
				...
				var urlPrepend = "http://www.asos.com/es/pgeproduct.aspx?";
				...
				fs.writeFileSync("linksES.json", '{"data":' + JSON.stringify(mergeCats(processArr)) + '}');
				//fs.writeFileSync("linksES.json", '{"data":' + JSON.stringify(eliminateDuplicates(resultsArr)) + '}');
				fs.writeFileSync("numTrackerES.json", '{"data":' + "0" + '}');
				writingToEndPoint(0);
				return;
			}
...
function writingToEndPoint(startFrom) {
	...
	var links = JSON.parse(fs.readFileSync("linksES.json", "utf-8"));
	...
	asyncLoop(start, function(loop) {
			...
			if (!error && response.statusCode == 200) {
				...
				if (get.writeToDB == true && get.method == "asos" && (jsonResults.length >= resultSpacing || l + startFrom == links.length - 1)) {
					Q(jsonResults).then(function(results) {
						...
						fs.writeFileSync("numTrackerES.json", '{"data":' + (l + startFrom) + '}');
						jsonResults = [];
						loop.next();
					});
		...
		cleanDB(0);
		callFinal = !callFinal;
		console.log(fs.existsSync('numTrackerES.json') + "," + fs.existsSync('linksES.json'));
		if (fs.existsSync('numTrackerES.json')) {
			fs.unlinkSync('numTrackerES.json');
		}
		if (fs.existsSync('linksES.json')) {
			fs.unlinkSync('linksES.json');
		}
...
//			fs.writeFileSync("numTrackerES.json", '{"data":' + inputCount + '}');
