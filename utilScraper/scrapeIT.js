var cheerio = require('cheerio'), email = require('emailjs'), get = require('./configIT.json'), mysql = require('mysql'), fs = require('fs'), request = require('request'), Q = require('q'), _ = require('lodash'), htmlencode = require('htmlencode');

var diff;
var requestCookie = "asos=topcatid=1000&PreferredSite=www.asos.com/it/&currencyid=19";
var timeStamp = 0;
var thresh = 0;
...
function timed() {
	try {
		absentConf = fs.existsSync('numTrackerIT.json');
	} catch(e) {
		absentConf = false;
		console.log(e);
	}
	if (absentConf == true) {
		try {
			absentConf = fs.existsSync('linksIT.json');
		} catch(e) {
			absentConf = false;
			console.log(e);
		}
	...
	if (absentConf == true && get.recover == true) {
		...
		var encode = JSON.parse(fs.readFileSync("numTrackerIT.json", "utf-8"));
		callFinal = !callFinal;
		if (encode.data > 0) {
...
function dataScrape(base, k, referenceArr) { {
		...
		if (get.method == "generic" || get.method == "asos") {
			if (k < get.searches.length - 1) {
				...
				var urlPrepend = "http://www.asos.com/it/pgeproduct.aspx?";
				...
				fs.writeFileSync("linksIT.json", '{"data":' + JSON.stringify(mergeCats(processArr)) + '}');
				//fs.writeFileSync("linksIT.json", '{"data":' + JSON.stringify(eliminateDuplicates(resultsArr)) + '}');
				fs.writeFileSync("numTrackerIT.json", '{"data":' + "0" + '}');
				writingToEndPoint(0);
				return;
			}
...
function writingToEndPoint(startFrom) {
	...
	var links = JSON.parse(fs.readFileSync("linksIT.json", "utf-8"));
	...
	asyncLoop(start, function(loop) {
			...
			if (!error && response.statusCode == 200) {
				...
				if (get.writeToDB == true && get.method == "asos" && (jsonResults.length >= resultSpacing || l + startFrom == links.length - 1)) {
					Q(jsonResults).then(function(results) {
						...
						fs.writeFileSync("numTrackerIT.json", '{"data":' + (l + startFrom) + '}');
						jsonResults = [];
						loop.next();
					});
		...
		cleanDB(0);
		callFinal = !callFinal;
		console.log(fs.existsSync('numTrackerIT.json') + "," + fs.existsSync('linksIT.json'));
		if (fs.existsSync('numTrackerIT.json')) {
			fs.unlinkSync('numTrackerIT.json');
		}
		if (fs.existsSync('linksIT.json')) {
			fs.unlinkSync('linksIT.json');
		}
...
//			fs.writeFileSync("numTrackerIT.json", '{"data":' + inputCount + '}');
