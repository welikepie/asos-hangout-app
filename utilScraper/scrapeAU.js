var cheerio = require('cheerio'), email = require('emailjs'), get = require('./configAU.json'), mysql = require('mysql'), fs = require('fs'), request = require('request'), Q = require('q'), _ = require('lodash'), htmlencode = require('htmlencode');

var diff;
var requestCookie = "asos=topcatid=1000&PreferredSite=www.asos.com/au/&currencyid=21";
var timeStamp = 0;
var thresh = 0;
...
function timed() {
	try {
		absentConf = fs.existsSync('numTrackerAU.json');
	} catch(e) {
		absentConf = false;
		console.log(e);
	}
	if (absentConf == true) {
		try {
			absentConf = fs.existsSync('linksAU.json');
		} catch(e) {
			absentConf = false;
			console.log(e);
		}
	...
	if (absentConf == true && get.recover == true) {
		...
		var encode = JSON.parse(fs.readFileSync("numTrackerAU.json", "utf-8"));
		callFinal = !callFinal;
		if (encode.data > 0) {
...
function dataScrape(base, k, referenceArr) { {
		...
		if (get.method == "generic" || get.method == "asos") {
			if (k < get.searches.length - 1) {
				...
				var urlPrepend = "http://www.asos.com/au/pgeproduct.aspx?";
				...
				fs.writeFileSync("linksAU.json", '{"data":' + JSON.stringify(mergeCats(processArr)) + '}');
				//fs.writeFileSync("linksAU.json", '{"data":' + JSON.stringify(eliminateDuplicates(resultsArr)) + '}');
				fs.writeFileSync("numTrackerAU.json", '{"data":' + "0" + '}');
				writingToEndPoint(0);
				return;
			}
...
function writingToEndPoint(startFrom) {
	...
	var links = JSON.parse(fs.readFileSync("linksAU.json", "utf-8"));
	...
	asyncLoop(start, function(loop) {
			...
			if (!error && response.statusCode == 200) {
				...
				if (get.writeToDB == true && get.method == "asos" && (jsonResults.length >= resultSpacing || l + startFrom == links.length - 1)) {
					Q(jsonResults).then(function(results) {
							...
							promise = promise.then(function() {
								return Q.all(_.map(product_group, function(product) {
									...
									return query('INSERT INTO products (' + 'id, timestamp, gender, ' + 'name, image, description' + ') VALUES (' + ':id, FROM_UNIXTIME(:timestamp), :gender, ' + ':name, :image, :description' + ') ON DUPLICATE KEY UPDATE id = :id, timestamp = FROM_UNIXTIME(:timestamp),gender=:gender, name = :name, description = :description, image = :image', product).then(function() {
										...
										// (assume GBP as currency for now; changed to AUD)
										return query('INSERT INTO product_prices (product_id, currency, price) ' + 'VALUES (:id, \'AUD\', :price)' + ' ON DUPLICATE KEY UPDATE product_id = :id, currency = \'AUD\', price=:price', product);
									}, function(error) {
						...
						fs.writeFileSync("numTrackerAU.json", '{"data":' + (l + startFrom) + '}');
						jsonResults = [];
						loop.next();
					});
		...
		cleanDB(0);
		callFinal = !callFinal;
		console.log(fs.existsSync('numTrackerAU.json') + "," + fs.existsSync('linksAU.json'));
		if (fs.existsSync('numTrackerAU.json')) {
			fs.unlinkSync('numTrackerAU.json');
		}
		if (fs.existsSync('linksAU.json')) {
			fs.unlinkSync('linksAU.json');
		}
...
//			fs.writeFileSync("numTrackerAU.json", '{"data":' + inputCount + '}');
